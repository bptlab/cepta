...
func setUpQueryServer(t *testing.T, listener *bufconn.Listener, mongoConfig libdb.MongoDBConfig) (*QueryServer, error) {
	server := NewQueryServer(mongoConfig)
	...
	go func() {
		if err := server.Serve(listener); err != nil {
			t.Fatalf("Query server failed to serve: %v", err)
		}
	}()
	return &server, nil
}
...
type Test struct {
	...
	userServer   *QueryServer
	userClient   pb.QueryServerClient
}
...
func (test *Test) setup(t *testing.T) *Test {
	...
	test.userServer, err = setUpQueryServer(t, userListener, libdb.MongoDBConfig{
	...
	test.userClient = pb.NewQueryClient(test.userEndpoint)
	return test
}
...
// Test ideas
// - Find a single transport
// - Test 3 more complex queries
