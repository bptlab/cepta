...
public class PlannedTrainDataReplayer extends PostgresReplayer<Long, PlannedTrainData> {
  ...
  @Override
  public PlannedTrainData convertToEvent(ResultSet result) {
    ...
    try {
      ...
      event.setPlannedTime(result.getLong("planned_time"));
      ...
      event.setPlannedDepartureReference(result.getLong("planned_departure_reference"));
      event.setPlannedArrivalReference(result.getLong("planned_arrival_reference"));
